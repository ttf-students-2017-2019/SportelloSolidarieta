...
public class SettingsController {
	...
	private Setting settings; // Settings loaded from the database
	...
	public SettingsController(MainCallback main) {
		this.main = main;
	}
	...
	void saveSetting(ActionEvent event) {
		...
		if (endTime.after(startTime)) {
			...
			try {
				...
				// If the inserted value is not compatible with appointment length and start end
				// time
		...
		// Updating the Settings and then save it to the db if end time is after start
		// time
	...
	private void bindHourAndSelectDeafult(ObservableList<ObservableHour> hourList,
		...
		// The hour list is in order so just taking the hour of day gets the item to
		// select
	...
	private void bindMinuteAndSelectDeafult(ObservableList<ObservableMinute> fromMinuteList,
		...
		// The minute list is in order so just taking the minutes and divide by 5 min
		// gap
	...
	private ObservableList<ObservableAppointmentLength> setupAppointmentLengthList() {
		...
		ObservableList<ObservableAppointmentLength> appointmentLengthObservable = FXCollections
				.<ObservableAppointmentLength>observableArrayList();
	...
	// Binding the From Hour combo-box and selecting the default setting
	private void bindApopointmentLengthAndSelectDeafult(
			ObservableList<ObservableAppointmentLength> appointmentLengthList) {
		id_appointment_length.setItems(appointmentLengthList);
		// The list is in order so just taking the minutes and divide by 5 min gap the
		// settings value -1
	...
	private void showAlertMaxAppointmentError(int maxAllowedAppointment) {
		...
		alert.setTitle("Messaggio di errore");
		alert.setHeaderText(
				"Il numero massimo di appuntamenti della giornata � maggiore del pi� alto valore consentito");
		alert.setContentText(
				"Il numero massimo di appuntamenti con le impostazioni correnti �: " + maxAllowedAppointment);
		alert.showAndWait();
	}
	...
	private void showAlertNotANumberMaxAppointmentError() {
		Alert alert = new Alert(AlertType.ERROR);
		alert.setTitle("Messaggio di errore");
		alert.setHeaderText("Il numero massimo di appuntamenti non � un numero");
